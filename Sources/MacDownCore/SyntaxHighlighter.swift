...
public enum SyntaxHighlighter {
    ...
    static func language(named name: String) -> Language? {
        ...
        case "js": languages["javascript"]
        case "ts": languages["typescript"]
        case "py": languages["python"]
        case "rb": languages["ruby"]
        case "golang": languages["go"]
        case "c++", "cxx": languages["cpp"]
        case "c#", "cs": languages["csharp"]
        case "objectivec", "obj-c": languages["objc"]
        default: languages[name.lowercased()]
        }
    }
    ...
    static func classifyIdentifier(_ id: String, _ lang: Language, remaining: String) -> String {
        if lang.keywordsCaseInsensitive, lang.keywords.contains(id.lowercased()) {
            return "kw"
        }
        if lang.keywords.contains(id) {
            return "kw"
        }
        if lang.capitalizedTypes, let first = id.first, first.isUppercase {
            return "ty"
        }
